...
class EMS_Leave_Request_CPT {
    ...
    public function filter_leave_request_row_actions( $actions, $post ) {
        if ( $post->post_type === 'leave_request' ) {
            ...
            // For approved, rejected, or cancelled statuses, remove edit links.
            // The "View" link (if it exists by default for non-public CPTs) might still appear or be handled by WordPress core.
            // The custom "Actions" column will handle status changes for pending items.
    ...
    public function add_leave_request_meta_boxes() {
        ...
        if ( $typenow === 'leave_request' ) { 
            ...
            if ( ($post_id && current_user_can('edit_leave_request', $post_id)) || current_user_can('publish_leave_requests') ) { 
                add_meta_box('leave_request_details_meta_box', __( 'Leave Request Details', 'emmansys' ), array( $this, 'render_leave_request_details_meta_box' ), 'leave_request', 'normal', 'high'); 
            } 
        }
    }
    ...
    public function save_leave_request_meta_data( $post_id, $post, $update ) {
        // ... (content remains the same as in version 1.2.4) ...
    ...
    public function render_leave_request_columns( $column, $post_id ) {
        ...
        switch ( $column ) { 
            case 'ems_leave_title_col': 
                // If status is approved or rejected, link to view, otherwise edit.
                // However, the filter_leave_request_row_actions should handle removing the main "Edit" link.
                // The title link itself usually goes to the edit screen. We can leave this as is,
                // as the edit screen itself will show fields as disabled if not editable.
            ...
            case 'ems_leave_actions': 
                ...
                if ( current_user_can( 'approve_leave_requests' ) && !in_array( $current_status, array( 'approved', 'rejected', 'cancelled' ) ) ) {
                    ...
                    foreach ( $statuses_map as $key => $label ) { 
                        // For pending requests, allow changing to approved, rejected.
                        // For draft requests, allow changing to pending, approved, rejected.
                        // Do not allow direct change to 'cancelled' from here if it's not already pending (employees cancel their own pending requests).
                        if ($key === 'cancelled' && $current_status !== 'pending') {
                             if ($current_status !== 'cancelled') continue; // Only show 'cancelled' if it's already cancelled (to revert if needed by mistake) or if it was pending
                        }
                    ...
                    echo '</div>'; 
                } elseif (in_array( $current_status, array( 'approved', 'rejected', 'cancelled' ) )) {
                     echo '<em>' . esc_html__( 'No actions available.', 'emmansys' ) . '</em>';
                } else { 
                    echo '—'; 
                } 
                break; 
