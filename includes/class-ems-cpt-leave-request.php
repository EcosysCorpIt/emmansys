...
class EMS_Leave_Request_CPT {
    ...
    public function __construct() {
        // Actions and filters specific to Leave Request CPT
    }
    ...
    public function register_leave_request_cpt() {
        ...
        $args = array(
            ...
            'capabilities' => array(
                ...
                // Omitting individual delete capabilities as 'delete_posts' covers them with map_meta_cap
            ),
            'map_meta_cap' => true,
            'has_archive' => false,
            'hierarchical' => false,
            ...
            'supports' => array( 'custom-fields' ), // No 'title' or 'editor'
        );
    ...
    public function add_leave_request_meta_boxes() {
        global $typenow;
        if ( $typenow === 'leave_request' ) {
            $post_id = isset($_GET['post']) ? intval($_GET['post']) : (isset($_POST['post_ID']) ? intval($_POST['post_ID']) : 0);
            if ( ($post_id && current_user_can('edit_leave_request', $post_id)) || current_user_can('publish_leave_requests') ) {
                 add_meta_box(
                    'leave_request_details_meta_box',
                    __( 'Leave Request Details', 'emmansys' ),
                    array( $this, 'render_leave_request_details_meta_box' ),
                    'leave_request',
                    'normal',
                    'high'
                );
    ...
    public function save_leave_request_meta_data( $post_id, $post, $update ) {
        // This method is identical to the one in version 1.1.20, just moved here.
        // It uses $this->has_overlapping_active_leave() and $this->generate_next_leave_request_id()
        // and $this->update_leave_balance_on_status_change()
        ...
        if ($employee_cpt_id_from_form && $start_date_val && $end_date_val && !$is_autosave) {
            ...
            if ($conflicting_leave_id) {
                ...
                $error_message = sprintf( __('Error: This leave request (Dates: %s to %s) overlaps with an existing active leave request: "%s" (ID: %s, Status: %s, Dates: %s - %s). The current leave request has been saved as Draft. Please cancel or modify the conflicting request, or change the dates/details of this request.', 'emmansys'), esc_html($start_date_val), esc_html($end_date_val), esc_html($conflicting_post_title), $conflicting_leave_id, esc_html($conflicting_status_label), esc_html(get_post_meta($conflicting_leave_id, '_leave_start_date', true)), esc_html(get_post_meta($conflicting_leave_id, '_leave_end_date', true)) );
                ...
                if (in_array($submitted_status, array('pending', 'approved'))) { $force_status_to_draft = true; }
        ...
        if ( isset( $_POST['ems_leave_employee_cpt_id'] ) ) {
            ...
            if ( $employee_cpt_id_for_meta > 0 ) {
                ...
                if ( $employee_post ) {
                    ...
                    if ( $linked_wp_user_id ) { update_post_meta( $post_id, '_leave_user_id', absint( $linked_wp_user_id ) ); } 
                    else { delete_post_meta( $post_id, '_leave_user_id'); }
                }
            } else {
                delete_post_meta( $post_id, '_leave_employee_cpt_id'); delete_post_meta( $post_id, '_leave_employee_name');
                if ( $post->post_author && !current_user_can('edit_others_leave_requests')) {
                    ...
                    if ($author_data) {
                        ...
                        if(!empty($author_employee_records)){ update_post_meta( $post_id, '_leave_employee_name', sanitize_text_field($author_employee_records[0]->post_title)); update_post_meta( $post_id, '_leave_employee_cpt_id', $author_employee_records[0]->ID); } 
                        else { update_post_meta( $post_id, '_leave_employee_name', $author_data->display_name); }
                    }
                } else { delete_post_meta( $post_id, '_leave_user_id'); }
            }
        }
        ...
        if ($force_status_to_draft) { $_POST['ems_leave_status'] = 'draft';}
        ...
        if ( $current_post_for_title && ( strtolower($current_post_for_title->post_title) === 'auto draft' || empty($current_post_for_title->post_title) || strpos($current_post_for_title->post_title, 'LR') !== 0 ) && !$is_autosave ) {
            $is_title_already_lr_format = strpos($current_post_for_title->post_title, 'LR') === 0;
            if (!$is_title_already_lr_format) {
                $new_lr_id_title = $this->generate_next_leave_request_id();
                if ($new_lr_id_title) {
                    remove_action('save_post_leave_request', array($this, 'save_leave_request_meta_data'), 20);
                    wp_update_post(array( 'ID' => $post_id, 'post_title' => $new_lr_id_title, 'post_name'  => sanitize_title($new_lr_id_title) ));
        ...
        $this->update_leave_balance_on_status_change($post_id, $old_status, $final_new_status);
    ...
    public function set_leave_request_columns( $columns ) {
        ...
        $new_columns['ems_leave_actions'] = __( 'Actions', 'emmansys' );
    ...
    public function sort_leave_request_columns_query( $query ) {
        ...
        if ($post_type === 'leave_request') {
            if ( 'ems_leave_employee_sort' === $orderby ) {
                $query->set( 'meta_key', '_leave_employee_name' );
                $query->set( 'orderby', 'meta_value' );
            } elseif ( 'ems_leave_user_sort' === $orderby ) {
                $query->set( 'meta_key', '_leave_user_id' );
                $query->set( 'orderby', 'meta_value_num' );
            } elseif ( 'ems_leave_type_sort' === $orderby ) {
                $query->set( 'meta_key', '_leave_type' );
                $query->set( 'orderby', 'meta_value' );
            } elseif ( 'ems_leave_status_sort' === $orderby ) {
                $query->set( 'meta_key', '_leave_status' );
                $query->set( 'orderby', 'meta_value' );
            }
        }
    }
    ...
    public function generate_next_leave_request_id() {
        $counter = get_option( 'ems_last_leave_request_id_counter', 0 );
        $counter++;
        $new_id_str = 'LR' . str_pad( (string)$counter, 8, '0', STR_PAD_LEFT );
        update_option( 'ems_last_leave_request_id_counter', $counter );
        return $new_id_str;
    }
    ...
    private function calculate_leave_days($start_date_str, $end_date_str, $duration_key) {
        ...
        if ($duration_key === 'half_day_am' || $duration_key === 'half_day_pm') { return 0.5; }
        elseif ($duration_key === 'whole_day') {
            try {
                $start_date = new DateTime($start_date_str);
                $end_date   = new DateTime($end_date_str);
                if ($start_date > $end_date) { return 0; }
                $end_date->modify('+1 day'); // Include the end date in calculation
    ...
    public function has_overlapping_active_leave($employee_cpt_id, $new_start_date_str, $new_end_date_str, $exclude_post_id = 0) {
        if (empty($employee_cpt_id) || empty($new_start_date_str) || empty($new_end_date_str)) { return false; }
        $args = array(
            'post_type'      => 'leave_request',
            'post_status'    => 'publish', // Assuming all leave requests are 'publish' status internally
            'posts_per_page' => -1,
            'meta_query'     => array(
                'relation' => 'AND',
                array( 'key' => '_leave_employee_cpt_id', 'value' => $employee_cpt_id, 'compare' => '=' ),
                array( 'key' => '_leave_status', 'value' => array('pending', 'approved'), 'compare' => 'IN' ),
            ),
        );
        if ($exclude_post_id > 0) { $args['post__not_in'] = array(intval($exclude_post_id)); }
        $existing_requests = get_posts($args);
        if (empty($existing_requests)) { return false; }
        try {
        ...
        foreach ($existing_requests as $request_post) {
            ...
            if (empty($existing_start_str) || empty($existing_end_str)) { continue; }
            try {
            ...
            if (($new_start_dt <= $existing_end_dt) && ($new_end_dt >= $existing_start_dt)) {
                return $request_post->ID; // Overlap found
            }
        }
    ...
    public function update_leave_balance_on_status_change($leave_request_id, $old_status, $new_status) {
        ...
        $days_for_this_request = $this->calculate_leave_days($leave_start_date, $leave_end_date, $leave_duration_key);
        ...
        $tracks_balance = $current_leave_type_definition && 
                          ( (isset($current_leave_type_definition['initial_balance']) && $current_leave_type_definition['initial_balance'] > 0) || $leave_type_key !== 'unpaid'); // Unpaid never tracks balance for deduction

        if (!$tracks_balance) { return false; }
        ...
        if ($new_status === 'approved' && $old_status !== 'approved') {
            ...
            // Approval revoked (e.g., changed to pending, rejected, cancelled): Add back previously deducted days
            if ($previously_deducted_days > 0) {
                $new_balance = $current_employee_balance + $previously_deducted_days;
                update_post_meta($employee_id, $balance_meta_key, $new_balance);
                delete_post_meta($leave_request_id, '_ems_deducted_leave_days'); // Clear the record of deducted days for this request
                return true;
            }
        } elseif ($new_status === 'approved' && $old_status === 'approved') {
            // Request was already approved, but details might have changed (e.g., dates, duration)
            // Adjust deduction based on new vs old deducted amount
            $new_deduction_amount = $days_for_this_request;
            if ($new_deduction_amount != $previously_deducted_days) {
                $adjustment = $previously_deducted_days - $new_deduction_amount; // If new is less, adjustment is positive (add back)
        ...
        return false; // No balance change occurred or needed
    }
}
