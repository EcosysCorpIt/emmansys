...
final class Employee_Management_System {
    ...
    private function __construct() {
        ...
        // Check dependencies before including other files or initializing hooks
    ...
    public function init_plugin_if_dependency_met() {
        // A different way to check dependency, might be better at plugins_loaded
        if ( ! function_exists( 'is_plugin_active' ) ) {
            include_once ABSPATH . 'wp-admin/includes/plugin.php';
        }
        if ( is_plugin_active( 'user-role-editor/user-role-editor.php' ) ) {
            $this->dependency_met = true;
            $this->includes(); // Only include files if dependency met
            $this->init_hooks(); // Only init hooks if dependency met
        } else {
            // Add notice if not met, admin_init might be too late for some checks but good for notice
    ...
    public function check_dependencies() {
        ...
        if ( is_plugin_active( 'user-role-editor/user-role-editor.php' ) ) {
            ...
            // Deactivate this plugin if User Role Editor is not active?
            // For now, just show a notice via plugins_loaded hook.
            // add_action( 'admin_notices', array( $this, 'dependency_missing_notice' ) );
        }
    }
    ...
    private function includes() {
        // Example: require_once EMS_PLUGIN_DIR . 'includes/class-ems-cpt.php';
    }
    ...
    /**
     * Plugin activation.
     * Roles and capabilities are now expected to be managed by User Role Editor.
     * This function will mainly register CPTs to ensure caps are known and flush rewrite rules.
     */
    public function activate() {
        // Ensure CPTs are registered so their capabilities are recognized by WordPress.
        // Admins will then need to use User Role Editor to assign these capabilities to roles.
    ...
    /**
     * Plugin deactivation.
     * Roles and capabilities are managed externally.
     */
    ...
    public function add_leave_request_meta_boxes() {
        // ... (same as v1.0.8 with capability check) ...
    ...
    public function render_leave_request_details_meta_box( $post ) {
        // ... (same as v1.0.8, including conditional employee dropdown based on 'edit_others_leave_requests') ...
        ...
        $leave_type = get_post_meta( $post->ID, '_leave_type', true ); $start_date = get_post_meta( $post->ID, '_leave_start_date', true );
        $end_date   = get_post_meta( $post->ID, '_leave_end_date', true ); $leave_reason = get_post_meta( $post->ID, '_leave_reason', true );
        $leave_status = get_post_meta( $post->ID, '_leave_status', true ); $admin_notes = get_post_meta( $post->ID, '_leave_admin_notes', true );
        $leave_types = $this->get_leave_types(); $statuses = $this->get_leave_statuses();
        ...
        <select id="ems_leave_employee_cpt_id" name="ems_leave_employee_cpt_id" required><option value=""><?php _e( '-- Select Employee --', 'emmansys' ); ?></option><?php foreach ( $employee_posts as $employee_post ) : ?><option value="<?php echo esc_attr( $employee_post->ID ); ?>" <?php selected( $selected_employee_cpt_id, $employee_post->ID ); ?>><?php echo esc_html( $employee_post->post_title ); ?><?php $linked_wp_user_id_option = get_post_meta($employee_post->ID, '_employee_user_id', true); if ($linked_wp_user_id_option) { echo ' (WP User ID: ' . esc_html($linked_wp_user_id_option) . ')'; } ?></option><?php endforeach; ?></select><p class="description"><?php _e( 'Select the employee filing this leave.', 'emmansys' ); ?></p>
    ...
    public function set_leave_request_columns( $columns ) { /* ... same as v1.0.7 ... */ unset($columns['title']); $new_columns = array(); $new_columns['cb'] = $columns['cb']; $new_columns['ems_leave_title_col'] = __( 'Leave Request', 'emmansys' ); $new_columns['ems_leave_employee'] = __( 'Employee', 'emmansys' ); $new_columns['ems_leave_user'] = __( 'WP User', 'emmansys' ); $new_columns['ems_leave_type'] = __( 'Leave Type', 'emmansys' ); $new_columns['ems_leave_dates'] = __( 'Dates', 'emmansys' ); $new_columns['ems_leave_status'] = __( 'Status', 'emmansys' ); $new_columns['date'] = $columns['date']; return $new_columns; }
    public function render_leave_request_columns( $column, $post_id ) { /* ... same as v1.0.7 ... */ $leave_types = $this->get_leave_types(); $statuses = $this->get_leave_statuses(); switch ( $column ) { case 'ems_leave_title_col': echo '<a class="row-title" href="' . esc_url(get_edit_post_link($post_id)) . '">' . esc_html(get_the_title($post_id)) . '</a>'; break; case 'ems_leave_employee': $employee_cpt_id = get_post_meta( $post_id, '_leave_employee_cpt_id', true ); $employee_name = get_post_meta( $post_id, '_leave_employee_name', true ); if ($employee_cpt_id && $employee_post = get_post($employee_cpt_id)) { echo '<a href="' . esc_url(get_edit_post_link($employee_cpt_id)) . '"><strong>' . esc_html($employee_post->post_title) . '</strong></a>'; } elseif ($employee_name) { echo '<strong>' . esc_html($employee_name) . '</strong>'; } else { echo '—'; } break; case 'ems_leave_user': $user_id = get_post_meta( $post_id, '_leave_user_id', true ); if ($user_id && $user = get_userdata($user_id)) { echo '<a href="' . esc_url(get_edit_user_link($user_id)) . '">' . esc_html($user->display_name) . '</a>'; } else { echo '—'; } break; case 'ems_leave_type': $type_key = get_post_meta( $post_id, '_leave_type', true ); echo esc_html( $leave_types[$type_key] ?? $type_key ); break; case 'ems_leave_dates': $start = get_post_meta( $post_id, '_leave_start_date', true ); $end = get_post_meta( $post_id, '_leave_end_date', true ); echo esc_html($start) . ' - ' . esc_html($end); break; case 'ems_leave_status': $status_key = get_post_meta( $post_id, '_leave_status', true ); echo '<strong>' . esc_html( $statuses[$status_key] ?? $status_key ) . '</strong>'; break; } }
    public function make_leave_request_columns_sortable( $columns ) { /* ... same as v1.0.7 ... */ $columns['ems_leave_title_col'] = 'title'; $columns['ems_leave_employee'] = 'ems_leave_employee_sort'; $columns['ems_leave_user'] = 'ems_leave_user_sort'; $columns['ems_leave_type'] = 'ems_leave_type_sort'; $columns['ems_leave_status'] = 'ems_leave_status_sort'; return $columns; }
    public function sort_leave_request_columns_query( $query ) { /* ... same as v1.0.7 ... */ if ( ! is_admin() || ! $query->is_main_query() ) return; $orderby = $query->get( 'orderby' ); $post_type = $query->get('post_type'); if ($post_type === 'leave_request') { if ( 'ems_leave_employee_sort' === $orderby ) { $query->set( 'meta_key', '_leave_employee_name' ); $query->set( 'orderby', 'meta_value' ); } elseif ( 'ems_leave_user_sort' === $orderby ) { $query->set( 'meta_key', '_leave_user_id' ); $query->set( 'orderby', 'meta_value_num' ); } elseif ( 'ems_leave_type_sort' === $orderby ) { $query->set( 'meta_key', '_leave_type' ); $query->set( 'orderby', 'meta_value' ); } elseif ( 'ems_leave_status_sort' === $orderby ) { $query->set( 'meta_key', '_leave_status' ); $query->set( 'orderby', 'meta_value' ); } } }

    // --- User Profile Leave Management Methods ---
    public function get_leave_types() { /* ... same as v1.0.7 ... */ return array( 'vacation' => __( 'Vacation', 'emmansys' ), 'sick' => __( 'Sick Leave', 'emmansys' ), 'personal' => __( 'Personal Leave', 'emmansys' ), 'unpaid' => __( 'Unpaid Leave', 'emmansys' ), 'other' => __( 'Other', 'emmansys' ),); }
    public function get_leave_statuses() { /* ... same as v1.0.7 ... */ return array( 'pending' => __( 'Pending', 'emmansys' ), 'approved' => __( 'Approved', 'emmansys' ), 'rejected' => __( 'Rejected', 'emmansys' ), 'cancelled' => __( 'Cancelled by Employee', 'emmansys' ),); }
    public function show_leave_management_on_profile( $user_profile ) { /* ... same as v1.0.7 with capability checks ... */ if ( !current_user_can('view_own_profile_leave_history') && get_current_user_id() != $user_profile->ID) { return; } if ( ! is_object( $user_profile ) || ! isset( $user_profile->ID ) ) return; $current_user_id = get_current_user_id(); $linked_employee_cpt_id = null; $employee_query_args = array('post_type' => 'employee', 'posts_per_page' => 1, 'meta_query' => array( array('key' => '_employee_user_id', 'value' => $user_profile->ID, 'compare' => '=')), 'fields' => 'ids'); $linked_employee_posts = get_posts($employee_query_args); if (!empty($linked_employee_posts)) { $linked_employee_cpt_id = $linked_employee_posts[0]; } ?> <hr><h2><?php _e( 'Leave Management', 'emmansys' ); ?></h2> <?php if ( $current_user_id === $user_profile->ID && $linked_employee_cpt_id && current_user_can('submit_profile_leave_request') ) : ?><h3><?php _e( 'File a New Leave Request', 'emmansys' ); ?></h3><form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>"><input type="hidden" name="action" value="ems_submit_leave_request"><input type="hidden" name="ems_user_id" value="<?php echo esc_attr( $user_profile->ID ); ?>"><input type="hidden" name="ems_employee_cpt_id_profile" value="<?php echo esc_attr( $linked_employee_cpt_id ); ?>"><?php wp_nonce_field( 'ems_submit_leave_request_nonce', 'ems_leave_request_profile_nonce' ); ?><table class="form-table"><tbody><tr><th><label for="ems_profile_leave_type"><?php _e( 'Leave Type', 'emmansys' ); ?></label></th><td><select id="ems_profile_leave_type" name="ems_profile_leave_type" required><option value=""><?php _e( '-- Select Type --', 'emmansys' ); ?></option><?php foreach ( $this->get_leave_types() as $key => $label ) : ?><option value="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></td></tr><tr><th><label for="ems_profile_start_date"><?php _e( 'Start Date', 'emmansys' ); ?></label></th><td><input type="date" id="ems_profile_start_date" name="ems_profile_start_date" class="regular-text" required /></td></tr><tr><th><label for="ems_profile_end_date"><?php _e( 'End Date', 'emmansys' ); ?></label></th><td><input type="date" id="ems_profile_end_date" name="ems_profile_end_date" class="regular-text" required /></td></tr><tr><th><label for="ems_profile_leave_reason_field"><?php _e( 'Reason for Leave', 'emmansys' ); ?></label></th><td><textarea id="ems_profile_leave_reason_field" name="ems_profile_leave_reason_field" rows="5" class="large-text" required></textarea></td></tr></tbody></table><?php submit_button( __( 'Submit Leave Request', 'emmansys' ) ); ?></form><hr style="margin: 20px 0;"><?php elseif ($current_user_id === $user_profile->ID && !$linked_employee_cpt_id && current_user_can('submit_profile_leave_request')): ?> <p><?php _e( 'To file leave requests from your profile, your WordPress user account must first be linked to an Employee record by an administrator.', 'emmansys' ); ?></p><hr style="margin: 20px 0;"><?php endif; ?><?php if (current_user_can('view_own_profile_leave_history') && get_current_user_id() == $user_profile->ID ) : ?><h3><?php _e( 'Your Leave Request History', 'emmansys' ); ?></h3><?php $args = array('post_type' => 'leave_request', 'posts_per_page' => -1, 'meta_query' => array('relation' => 'OR', array( 'key' => '_leave_employee_cpt_id', 'value' => $linked_employee_cpt_id, 'compare' => '=', 'type' => 'NUMERIC'), array( 'key' => '_leave_user_id', 'value' => $user_profile->ID, 'compare' => '=', 'type' => 'NUMERIC')), 'orderby' => 'date', 'order' => 'DESC',); if (!$linked_employee_cpt_id) { $args['meta_query'] = array(array( 'key' => '_leave_user_id', 'value' => $user_profile->ID, 'compare' => '=', 'type' => 'NUMERIC'));} $user_leave_requests = new WP_Query( $args ); if ( $user_leave_requests->have_posts() ) : ?><table class="wp-list-table widefat fixed striped"><thead><tr><th><?php _e( 'Request Date', 'emmansys' ); ?></th><th><?php _e( 'Leave Type', 'emmansys' ); ?></th><th><?php _e( 'Start Date', 'emmansys' ); ?></th><th><?php _e( 'End Date', 'emmansys' ); ?></th><th><?php _e( 'Status', 'emmansys' ); ?></th><th><?php _e( 'Reason', 'emmansys' ); ?></th></tr></thead><tbody><?php $leave_types_map = $this->get_leave_types(); $statuses_map = $this->get_leave_statuses(); while ( $user_leave_requests->have_posts() ) : $user_leave_requests->the_post(); $request_id = get_the_ID(); ?><tr><td><?php echo get_the_date( '', $request_id ); ?></td><td><?php echo esc_html( $leave_types_map[get_post_meta( $request_id, '_leave_type', true )] ?? get_post_meta( $request_id, '_leave_type', true ) ); ?></td><td><?php echo esc_html( get_post_meta( $request_id, '_leave_start_date', true ) ); ?></td><td><?php echo esc_html( get_post_meta( $request_id, '_leave_end_date', true ) ); ?></td><td><strong><?php echo esc_html( $statuses_map[get_post_meta( $request_id, '_leave_status', true )] ?? get_post_meta( $request_id, '_leave_status', true ) ); ?></strong></td><td><?php echo wp_kses_post( get_post_meta( $request_id, '_leave_reason', true ) ); ?></td></tr><?php endwhile; ?></tbody></table><?php else : ?> <p><?php _e( 'You have not submitted any leave requests yet.', 'emmansys' ); ?></p><?php endif; wp_reset_postdata(); ?><?php endif; ?><hr style="margin-top:20px;"><?php }
    public function handle_profile_leave_request_submission() { /* ... same as v1.0.7 with capability check ... */ if ( !current_user_can('submit_profile_leave_request') ) { wp_die( __( 'You do not have permission to submit leave requests.', 'emmansys' ) ); } if ( ! isset( $_POST['ems_leave_request_profile_nonce'], $_POST['ems_user_id'], $_POST['ems_employee_cpt_id_profile'] ) || ! wp_verify_nonce( $_POST['ems_leave_request_profile_nonce'], 'ems_submit_leave_request_nonce' ) || get_current_user_id() != $_POST['ems_user_id'] ) { wp_die( __( 'Security check failed or you are not authorized to perform this action.', 'emmansys' ) ); } $user_id = absint( $_POST['ems_user_id'] ); $employee_cpt_id = absint( $_POST['ems_employee_cpt_id_profile'] ); $user_info = get_userdata( $user_id ); $employee_info = get_post( $employee_cpt_id ); if ( ! $user_info || ! $employee_info || $employee_info->post_type !== 'employee' ) { set_transient( 'ems_leave_notice_error', __( 'Invalid user or employee record for leave request.', 'emmansys' ), 60 ); wp_safe_redirect( wp_get_referer() ?: get_edit_user_link( $user_id ) ); exit; } $linked_user_on_employee_cpt = get_post_meta($employee_cpt_id, '_employee_user_id', true); if (absint($linked_user_on_employee_cpt) !== $user_id) { set_transient( 'ems_leave_notice_error', __( 'Employee record mismatch.', 'emmansys' ), 60 ); wp_safe_redirect( wp_get_referer() ?: get_edit_user_link( $user_id ) ); exit; } $leave_type = isset( $_POST['ems_profile_leave_type'] ) ? sanitize_key( $_POST['ems_profile_leave_type'] ) : ''; $start_date = isset( $_POST['ems_profile_start_date'] ) ? sanitize_text_field( $_POST['ems_profile_start_date'] ) : ''; $end_date = isset( $_POST['ems_profile_end_date'] ) ? sanitize_text_field( $_POST['ems_profile_end_date'] ) : ''; $leave_reason = isset( $_POST['ems_profile_leave_reason_field'] ) ? sanitize_textarea_field( $_POST['ems_profile_leave_reason_field'] ) : ''; if ( empty( $leave_type ) || empty( $start_date ) || empty( $end_date ) || empty( $leave_reason ) ) { set_transient( 'ems_leave_notice_error', __( 'All fields are required for leave submission.', 'emmansys' ), 60 ); wp_safe_redirect( wp_get_referer() ?: get_edit_user_link( $user_id ) ); exit; } if ( strtotime($end_date) < strtotime($start_date) ) { set_transient( 'ems_leave_notice_error', __( 'End date cannot be before start date.', 'emmansys' ), 60 ); wp_safe_redirect( wp_get_referer() ?: get_edit_user_link( $user_id ) ); exit; } $post_title = sprintf( __( 'Leave: %s (%s to %s)', 'emmansys' ), $employee_info->post_title, $start_date, $end_date ); $leave_request_data = array( 'post_title' => $post_title, 'post_status' => 'publish', 'post_type' => 'leave_request', 'post_author' => $user_id, ); $new_leave_request_id = wp_insert_post( $leave_request_data, true ); if ( is_wp_error( $new_leave_request_id ) ) { set_transient( 'ems_leave_notice_error', __( 'Failed to submit leave request: ', 'emmansys' ) . $new_leave_request_id->get_error_message(), 60 ); } else { update_post_meta( $new_leave_request_id, '_leave_employee_cpt_id', $employee_cpt_id ); update_post_meta( $new_leave_request_id, '_leave_user_id', $user_id ); update_post_meta( $new_leave_request_id, '_leave_employee_name', sanitize_text_field($employee_info->post_title) ); update_post_meta( $new_leave_request_id, '_leave_type', $leave_type ); update_post_meta( $new_leave_request_id, '_leave_start_date', $start_date ); update_post_meta( $new_leave_request_id, '_leave_end_date', $end_date ); update_post_meta( $new_leave_request_id, '_leave_reason', $leave_reason ); update_post_meta( $new_leave_request_id, '_leave_status', 'pending' ); set_transient( 'ems_leave_notice_success', __( 'Leave request submitted successfully. It is now pending approval.', 'emmansys' ), 60 ); } wp_safe_redirect( wp_get_referer() ?: get_edit_user_link( $user_id ) ); exit; }
