...
final class Employee_Management_System {
     ...
     * @var string
     */
    const VERSION = '1.1.20'; // Incremented version
     ...
     * @var string
     */
    private $add_new_leave_page_hook_suffix = '';
    ...
    private function init_hooks() {
        ...
        add_action( 'admin_post_ems_admin_add_new_leave', array( $this, 'handle_admin_add_new_leave_request' ) );
        add_action( 'wp_ajax_ems_submit_profile_leave', array( $this, 'ajax_handle_profile_leave_submission' ) );
        add_action( 'wp_ajax_ems_change_leave_status', array( $this, 'ajax_ems_change_leave_status' ) );
        ...
        // Forcefully hide the 'Add New' button on the Leave Request CPT list table via CSS
        add_action( 'admin_head', array( $this, 'hide_leave_request_add_new_button_css' ) );
    }
    ...
    public function add_admin_menus() {
        $this->dashboard_page_hook_suffix = add_menu_page(
            ...
            __( 'My Dashboard', 'emmansys' ),
            'submit_profile_leave_request', // Capability for employees to see their dashboard
        ...
        $this->add_new_leave_page_hook_suffix = add_submenu_page(
            'edit.php?post_type=leave_request',    // Parent slug (Leave Requests CPT list table)
            __( 'Add New Leave Request', 'emmansys' ), // Page title
            __( 'Add New Leave', 'emmansys' ),        // Menu title for this custom "Add New" page
            'publish_leave_requests', // General capability to add any leave request (admin/manager or self)
            'ems-add-new-leave-request',            // Menu slug for this custom page
            array( $this, 'render_add_new_leave_request_page' ) // Callback function to render the page
        );
    ...
    public function enqueue_admin_scripts( $hook_suffix ) {
        ...
        if ( 'post.php' === $hook_suffix || 'post-new.php' === $hook_suffix || $hook_suffix === $this->add_new_leave_page_hook_suffix ) {
            $screen = get_current_screen();
            // Check if we are on a leave_request CPT screen OR our custom add new page
            if ( ($screen && 'leave_request' === $screen->post_type) || $hook_suffix === $this->add_new_leave_page_hook_suffix ) {
        ...
        if ($hook_suffix === $this->leave_types_page_hook_suffix || $hook_suffix === $this->add_new_leave_page_hook_suffix) {
            wp_enqueue_style( 'wp-admin' ); // For general admin styling
        }

        if ( 'edit.php' === $pagenow && isset($_GET['post_type']) && $_GET['post_type'] === 'leave_request' ) {
            wp_enqueue_script(
                'ems-admin-leave-list-actions',
                EMS_PLUGIN_URL . 'js/admin-leave-list-actions.js',
                array( 'jquery' ),
                self::VERSION,
                true
            );
     ...
     * Plugin activation.
     */
    public function activate() { /* ... same as 1.1.19 ... */ $this->register_employee_cpt(); $this->register_leave_request_cpt(); flush_rewrite_rules(); $js_dir = EMS_PLUGIN_DIR . 'js/'; if (!is_dir($js_dir)) { wp_mkdir_p($js_dir); } if (!file_exists($js_dir . 'admin-leave-validation.js')) { file_put_contents($js_dir . 'admin-leave-validation.js', '// Admin Leave Validation Script'); } if (!file_exists($js_dir . 'profile-leave-validation.js')) { file_put_contents($js_dir . 'profile-leave-validation.js', '// Profile/Dashboard Leave Validation Script'); } if (!file_exists($js_dir . 'admin-leave-list-actions.js')) { file_put_contents($js_dir . 'admin-leave-list-actions.js', '// Admin Leave List Actions Script'); } if ( false === get_option( 'ems_last_leave_request_id_counter' ) ) { add_option( 'ems_last_leave_request_id_counter', 0 ); } }

    /**
     * Plugin deactivation.
     */
    public function deactivate() { /* ... same as 1.1.19 ... */ flush_rewrite_rules(); }
    ...
    public function register_leave_request_cpt() {
        $labels = array(
            ...
            'add_new' => '', // Intentionally blank to hide the "Add New" button. The custom "Add New Leave" submenu page and CSS hide rule are used instead.
            // 'add_new_item' => __( 'Add New Leave Request', 'emmansys' ), // Using custom page
            'new_item' => __( 'New Leave Request', 'emmansys' ),
            'edit_item' => __( 'Edit Leave Request', 'emmansys' ),
        ...
        $args = array(
            ...
            'capabilities' => array(
                ...
                'delete_posts' => 'delete_leave_requests',
                'delete_private_posts' => 'delete_private_leave_requests',
                'delete_published_posts' => 'delete_published_leave_requests',
                'delete_others_posts' => 'delete_others_leave_requests',
                'edit_private_posts' => 'edit_private_leave_requests',
    ...
    public function render_leave_request_details_meta_box( $post ) {  /* ... same as v1.1.19 ... */
        ...
        $today = current_time('Y-m-d');
        $min_date_attr = (current_user_can('approve_leave_requests')) ? '' : 'min="' . esc_attr($today) . '"';
        
        $display_title = $post->post_title;
        if (strpos(strtolower($display_title), 'auto draft') !== false || empty($display_title) || strpos($display_title, 'LR') !== 0) {
            $display_title = __('(Title will be auto-generated: LR#######)', 'emmansys');
        }

        ?>
        <p><em><?php printf(__( 'Leave Request Title: %s', 'emmansys'), esc_html($display_title)); ?></em></p>
    ...
    public function save_leave_request_meta_data( $post_id, $post, $update ) { /* ... same as v1.1.19 ... */
        ...
        if ( !current_user_can('approve_leave_requests') && !empty($start_date_val) && $start_date_val < $today_val && !$update ) {
        ...
        $force_status_to_draft = false;
        $redirect_to_edit_after_save = false;
        $submitted_status = isset($_POST['ems_leave_status']) ? sanitize_key($_POST['ems_leave_status']) : ($update ? $old_status : 'pending');

        if ($employee_cpt_id_from_form && $start_date_val && $end_date_val && !$is_autosave) {
            ...
            if ($conflicting_leave_id) {
                $conflicting_post_title = get_the_title($conflicting_leave_id);
                $conflicting_status_key = get_post_meta($conflicting_leave_id, '_leave_status', true);
                $statuses_map = EMS_Leave_Options::get_leave_statuses();
                $conflicting_status_label = $statuses_map[$conflicting_status_key] ?? esc_html($conflicting_status_key);

                $error_message = sprintf(
                    __('Error: This leave request (Dates: %s to %s) overlaps with an existing active leave request: "%s" (ID: %s, Status: %s, Dates: %s - %s). The current leave request has been saved as Draft. Please cancel or modify the conflicting request, or change the dates/details of this request.', 'emmansys'),
                    esc_html($start_date_val),
                    esc_html($end_date_val),
                    esc_html($conflicting_post_title),
                    $conflicting_leave_id,
                    esc_html($conflicting_status_label),
                    esc_html(get_post_meta($conflicting_leave_id, '_leave_start_date', true)),
                    esc_html(get_post_meta($conflicting_leave_id, '_leave_end_date', true))
                );
                add_settings_error('ems_leave_notice', 'overlap_error_admin_forced_draft', $error_message, 'error'); // Use 'ems_leave_notice' for CPT edit screen
                set_transient('settings_errors', get_settings_errors('ems_leave_notice'), 30); // Store only 'ems_leave_notice' errors
                
                if (in_array($submitted_status, array('pending', 'approved'))) {
                    $force_status_to_draft = true;
                }
                $redirect_to_edit_after_save = true;
            }
        }
        ...
        if ($force_status_to_draft) {
            $_POST['ems_leave_status'] = 'draft';
        }
        ...
        foreach ( $other_fields_to_save as $meta_key => $field_config ) {
            if ( isset( $_POST[ $field_config['field_name'] ] ) ) {
                $value_to_save = $_POST[ $field_config['field_name'] ];
                $sanitized_value = call_user_func( $field_config['sanitize_callback'], $value_to_save );
                update_post_meta( $post_id, $meta_key, $sanitized_value );
            }
        }

        if (!current_user_can('approve_leave_requests') && $old_status === 'pending' && isset($_POST['ems_leave_status']) && sanitize_key($_POST['ems_leave_status']) === 'cancelled' && !$force_status_to_draft) {
             update_post_meta($post_id, '_leave_status', 'cancelled');
        }
        
        $current_post_for_title = get_post($post_id);
        if ( $current_post_for_title &&
             ( strtolower($current_post_for_title->post_title) === 'auto draft' ||
               empty($current_post_for_title->post_title) ||
               strpos($current_post_for_title->post_title, 'LR') !== 0
             ) &&
             !$is_autosave
           ) {
            $is_title_already_lr_format = strpos($current_post_for_title->post_title, 'LR') === 0;

            if (!$is_title_already_lr_format) {
                $new_lr_id_title = $this->_generate_next_leave_request_id();
                if ($new_lr_id_title) {
                    remove_action('save_post_leave_request', array($this, 'save_leave_request_meta_data'), 20);
                    wp_update_post(array(
                        'ID'         => $post_id,
                        'post_title' => $new_lr_id_title,
                        'post_name'  => sanitize_title($new_lr_id_title)
                    ));
                    add_action('save_post_leave_request', array($this, 'save_leave_request_meta_data'), 20, 3);
                }
            }
    ...
    public function ajax_handle_profile_leave_submission() { /* ... same as v1.1.19 ... */
        ...
        $post_title = $this->_generate_next_leave_request_id();
        if (!$post_title) {
            $post_title = __('Leave Request', 'emmansys') . ' ' . time(); // Fallback
            error_log("EmManSys: Failed to generate auto-increment ID for leave request during AJAX submission.");
        }
     ...
     * @since 1.1.16 (Modified 1.1.20 for employee field restriction)
     */
    public function render_add_new_leave_request_page() {
        if ( ! current_user_can( 'publish_leave_requests' ) ) { 
            wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'emmansys' ) );
        }

        $can_select_others_employee = current_user_can('edit_others_leave_requests');
        $current_wp_user_id = get_current_user_id();
        $linked_employee_cpt_id_for_user = null;
        $employee_name_for_user = '';
        $form_submission_blocked_message = '';

        if (!$can_select_others_employee) {
            // User can only submit for themselves, try to find their linked employee record
            $args_user_employee = array(
                'post_type' => 'employee',
                'posts_per_page' => 1,
                'meta_query' => array(
                    array(
                        'key' => '_employee_user_id',
                        'value' => $current_wp_user_id,
                        'compare' => '='
                    )
                ),
                'fields' => 'ids' 
            ...
            if (!empty($current_user_employee_records)) {
                $linked_employee_cpt_id_for_user = $current_user_employee_records[0];
                $employee_post_for_user = get_post($linked_employee_cpt_id_for_user);
                if ($employee_post_for_user) {
                    $employee_name_for_user = $employee_post_for_user->post_title;
                }
            } else {
                // User is not linked to an employee record
                $form_submission_blocked_message = __('You are not linked to an Employee record. Please contact an administrator to link your user account before submitting leave requests.', 'emmansys');
            }
        }

        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Add New Leave Request', 'emmansys' ); ?></h1>
            <?php 
            // Display specific overlap error first, if any
            $overlap_error = get_transient('ems_admin_leave_overlap_error');
            if ( $overlap_error ) {
                echo '<div class="notice notice-error is-dismissible"><p>' . esc_html( $overlap_error ) . '</p></div>';
                delete_transient('ems_admin_leave_overlap_error');
            }

            // Display other validation errors (e.g., empty fields) from the 'settings_errors' transient
            settings_errors('ems_add_leave_notice'); 
            
            // Display message if form submission is blocked (e.g., user not linked)
            if ( !empty($form_submission_blocked_message) ) {
                echo '<div class="notice notice-warning is-dismissible"><p>' . esc_html( $form_submission_blocked_message ) . '</p></div>';
            }
            
            $submitted_data = get_transient('ems_add_leave_form_data');
            if ($submitted_data) {
                delete_transient('ems_add_leave_form_data');
            } else {
                $submitted_data = array(); 
            }
            ?>
            <form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>" id="ems-admin-add-new-leave-form">
                <input type="hidden" name="action" value="ems_admin_add_new_leave">
                <?php wp_nonce_field( 'ems_admin_add_new_leave_nonce', '_wpnonce_ems_admin_add_new_leave' ); ?>

                <table class="form-table">
                    <tbody>
                        <tr>
                            <th scope="row"><label for="ems_leave_employee_cpt_id"><?php esc_html_e( 'Employee', 'emmansys' ); ?></label></th>
                            <td>
                                <?php if ($can_select_others_employee) : ?>
                                    <?php
                                    ...
                                    $selected_employee_cpt_id = $submitted_data['ems_leave_employee_cpt_id'] ?? '';
                                    ?>
                                    <select id="ems_leave_employee_cpt_id" name="ems_leave_employee_cpt_id" required>
                                        <option value=""><?php esc_html_e( '-- Select Employee --', 'emmansys' ); ?></option>
                                        <?php foreach ( $employee_posts as $employee_post ) : ?>
                                            <option value="<?php echo esc_attr( $employee_post->ID ); ?>" <?php selected( $selected_employee_cpt_id, $employee_post->ID ); ?>>
                                                <?php echo esc_html( $employee_post->post_title ); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                    <p class="description"><?php esc_html_e( 'Select the employee filing this leave.', 'emmansys' ); ?></p>
                                <?php elseif ($linked_employee_cpt_id_for_user) : ?>
                                    <input type="hidden" name="ems_leave_employee_cpt_id" value="<?php echo esc_attr($linked_employee_cpt_id_for_user); ?>" />
                                    <strong><?php echo esc_html($employee_name_for_user); ?></strong>
                                    <p class="description"><?php esc_html_e( 'Leave will be submitted for your linked employee record.', 'emmansys' ); ?></p>
                                <?php else : ?>
                                    <?php // This case is for non-admins who are NOT linked to an employee. The $form_submission_blocked_message is shown above. ?>
                                    <p><em><?php esc_html_e( 'Employee selection is not available.', 'emmansys' ); ?></em></p>
                                <?php endif; ?>
                            </td>
                        </tr>
                        ...
                        <?php
                        $all_leave_types = EMS_Leave_Options::get_leave_types();
                        $selected_leave_type = $submitted_data['ems_leave_type'] ?? '';
                        ?>
                        <tr>
                            <th scope="row"><label for="ems_leave_type"><?php esc_html_e( 'Leave Type', 'emmansys' ); ?></label></th>
                            <td>
                                <select id="ems_leave_type" name="ems_leave_type" required <?php if (!empty($form_submission_blocked_message)) echo 'disabled'; ?>>
                                    <option value=""><?php esc_html_e( '-- Select Type --', 'emmansys' ); ?></option>
                                    <?php foreach ( $all_leave_types as $key => $type_data ) : ?>
                                        <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $selected_leave_type, $key ); ?>>
                        ...
                        $today = current_time('Y-m-d');
                        $min_date_attr = (current_user_can('approve_leave_requests')) ? '' : 'min="' . esc_attr($today) . '"';
                        $start_date_val = $submitted_data['ems_leave_start_date'] ?? '';
                        $end_date_val = $submitted_data['ems_leave_end_date'] ?? '';
                        ?>
                        <tr>
                            <th scope="row"><label for="ems_leave_start_date"><?php esc_html_e( 'Start Date', 'emmansys' ); ?></label></th>
                            <td><input type="date" id="ems_leave_start_date" name="ems_leave_start_date" value="<?php echo esc_attr($start_date_val); ?>" class="regular-text ems-leave-start-date" <?php echo $min_date_attr; ?> required <?php if (!empty($form_submission_blocked_message)) echo 'disabled'; ?>/></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="ems_leave_end_date"><?php esc_html_e( 'End Date', 'emmansys' ); ?></label></th>
                            <td><input type="date" id="ems_leave_end_date" name="ems_leave_end_date" value="<?php echo esc_attr($end_date_val); ?>" class="regular-text ems-leave-end-date" <?php echo $min_date_attr; ?> required <?php if (!empty($form_submission_blocked_message)) echo 'disabled'; ?>/></td>
                        </tr>

                        <?php
                        $durations = EMS_Leave_Options::get_leave_durations();
                        $selected_duration = $submitted_data['ems_leave_duration'] ?? 'whole_day';
                        ?>
                        <tr>
                            <th scope="row"><label for="ems_leave_duration"><?php esc_html_e( 'Leave Duration', 'emmansys' ); ?></label></th>
                            <td>
                                <select id="ems_leave_duration" name="ems_leave_duration" required <?php if (!empty($form_submission_blocked_message)) echo 'disabled'; ?>>
                                    <?php foreach ( $durations as $key => $label ) : ?>
                                        <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $selected_duration, $key ); ?>>
                        ...
                        <?php $reason_val = $submitted_data['ems_leave_reason_field'] ?? ''; ?>
                        <tr>
                            <th scope="row"><label for="ems_leave_reason_field"><?php esc_html_e( 'Reason for Leave', 'emmansys' ); ?></label></th>
                            <td><textarea id="ems_leave_reason_field" name="ems_leave_reason_field" rows="5" class="large-text" required <?php if (!empty($form_submission_blocked_message)) echo 'disabled'; ?>><?php echo esc_textarea($reason_val); ?></textarea></td>
                        </tr>
                        
                        <?php
                        $statuses = EMS_Leave_Options::get_leave_statuses();
                        $selected_status = $submitted_data['ems_leave_status'] ?? 'pending';
                        ?>
                         <tr>
                            <th scope="row"><label for="ems_leave_status"><?php esc_html_e( 'Leave Status', 'emmansys' ); ?></label></th>
                            <td>
                                <select id="ems_leave_status" name="ems_leave_status" <?php if (!empty($form_submission_blocked_message)) echo 'disabled'; ?>>
                                    <?php foreach ( $statuses as $key => $label ) : 
                                        // For new requests, only allow 'pending' or 'approved' (if admin is creating)
                                        // If form is repopulated due to error, allow the previously selected status.
                                        if ( !current_user_can('approve_leave_requests') && $key === 'approved' && $selected_status !== 'approved') continue;
                                        if ( $key === 'rejected' && $selected_status !== 'rejected') continue; // Typically not set on creation
                                        if ( $key === 'cancelled' && $selected_status !== 'cancelled') continue; // Typically not set on creation
                                        
                                        // Default to 'pending' if no previous selection and not admin.
                                        $current_selection = $selected_status;
                                        if (empty($current_selection) && !current_user_can('approve_leave_requests')) {
                                            $current_selection = 'pending';
                                        }
                                    ?>
                                        <option value="<?php echo esc_attr( $key ); ?>" <?php selected( $current_selection, $key ); ?>>
                                            <?php echo esc_html( $label ); ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                                <p class="description"><?php esc_html_e('Default is "Pending". Admins can set to "Approved" directly if needed.', 'emmansys'); ?></p>
                            </td>
                        </tr>
                         <tr>
                            <th scope="row"><label for="ems_leave_admin_notes"><?php esc_html_e( 'Admin Notes (Optional)', 'emmansys' ); ?></label></th>
                            <td><textarea id="ems_leave_admin_notes" name="ems_leave_admin_notes" rows="3" class="large-text" <?php if (!empty($form_submission_blocked_message)) echo 'disabled'; ?>><?php echo esc_textarea($submitted_data['ems_leave_admin_notes'] ?? ''); ?></textarea></td>
                        </tr>
                    </tbody>
                </table>
                <?php 
                ...
                submit_button( __( 'Submit Leave Request', 'emmansys' ), 'primary', 'submit', true, $submit_button_attributes ); 
                ?>
            </form>
        </div>
        <?php
     ...
     * @since 1.1.16 (Modified 1.1.19 for better overlap notification, 1.1.20 for employee field handling)
     */
    public function handle_admin_add_new_leave_request() {
        if ( ! isset( $_POST['_wpnonce_ems_admin_add_new_leave'] ) || ! wp_verify_nonce( $_POST['_wpnonce_ems_admin_add_new_leave'], 'ems_admin_add_new_leave_nonce' ) ) {
            wp_die( esc_html__( 'Security check failed.', 'emmansys' ) );
        }
        if ( ! current_user_can( 'publish_leave_requests' ) ) { 
            wp_die( esc_html__( 'You do not have permission to create leave requests.', 'emmansys' ) );
        }

        $errors = array();
        $form_data = $_POST; 
        $overlap_error_occurred = false;

        // Determine employee CPT ID based on user capability
        $can_select_others_employee = current_user_can('edit_others_leave_requests');
        $employee_cpt_id = 0;

        if ($can_select_others_employee) {
            $employee_cpt_id = isset( $_POST['ems_leave_employee_cpt_id'] ) ? absint( $_POST['ems_leave_employee_cpt_id'] ) : 0;
        } else {
            // User cannot select others, so their own linked employee ID should have been in a hidden field
            // or derived if the hidden field was somehow bypassed (though UI should prevent this).
             $current_wp_user_id = get_current_user_id();
             $args_user_employee = array(
                'post_type' => 'employee',
                'posts_per_page' => 1,
                'meta_query' => array( array( 'key' => '_employee_user_id', 'value' => $current_wp_user_id, 'compare' => '=' ) ),
                'fields' => 'ids'
            );
            $current_user_employee_records = get_posts($args_user_employee);
            if (!empty($current_user_employee_records)) {
                $employee_cpt_id = $current_user_employee_records[0];
                // If the form POSTed an ID, it should match this one for non-admins.
                // If form did not POST (e.g. hidden field missing), this derived ID is used.
                // If form POSTed a different ID (tampering), this derived ID takes precedence for non-admins.
                $form_data['ems_leave_employee_cpt_id'] = $employee_cpt_id; // Ensure form_data reflects the correct ID for repopulation
            }
        }
        
        // If after all checks, employee_cpt_id is still 0, it's an error (e.g. non-admin not linked)
        if ( empty( $employee_cpt_id ) ) {
             if (!$can_select_others_employee) {
                // This specific message is for non-admins who aren't linked.
                // For admins, the generic "Employee is required" will be caught below.
                $errors[] = __('You must be linked to an Employee record to submit a leave request. Please contact an administrator.', 'emmansys');
             } else {
                // For admins, if they didn't select one from dropdown
                 $errors[] = __( 'Employee is required.', 'emmansys' );
             }
        }


        $leave_type_key  = isset( $_POST['ems_leave_type'] ) ? sanitize_key( $_POST['ems_leave_type'] ) : '';
        $start_date_val  = isset( $_POST['ems_leave_start_date'] ) ? sanitize_text_field( $_POST['ems_leave_start_date'] ) : '';
        $end_date_val    = isset( $_POST['ems_leave_end_date'] ) ? sanitize_text_field( $_POST['ems_leave_end_date'] ) : '';
        $duration_key    = isset( $_POST['ems_leave_duration'] ) ? sanitize_key( $_POST['ems_leave_duration'] ) : '';
        $reason          = isset( $_POST['ems_leave_reason_field'] ) ? sanitize_textarea_field( $_POST['ems_leave_reason_field'] ) : '';
        $status          = isset( $_POST['ems_leave_status'] ) ? sanitize_key( $_POST['ems_leave_status'] ) : 'pending'; 
        $admin_notes     = isset( $_POST['ems_leave_admin_notes'] ) ? sanitize_textarea_field( $_POST['ems_leave_admin_notes'] ) : '';

        // Initial field validations
        if ( empty( $leave_type_key ) ) $errors[] = __( 'Leave Type is required.', 'emmansys' );
        if ( empty( $start_date_val ) ) $errors[] = __( 'Start Date is required.', 'emmansys' );
        if ( empty( $end_date_val ) ) $errors[] = __( 'End Date is required.', 'emmansys' );
        if ( empty( $duration_key ) ) $errors[] = __( 'Leave Duration is required.', 'emmansys' );
        if ( empty( $reason ) ) $errors[] = __( 'Reason for Leave is required.', 'emmansys' );
        
        if ( !empty($start_date_val) && !empty($end_date_val) && $end_date_val < $start_date_val ) {
            $errors[] = __( 'End date cannot be earlier than the start date.', 'emmansys' );
        }

        // Only check for overlap if no other basic validation errors from initial checks
        if (empty($errors) && $employee_cpt_id) { // Ensure employee_cpt_id is valid before overlap check
            $conflicting_leave_id = $this->has_overlapping_active_leave($employee_cpt_id, $start_date_val, $end_date_val);
            if ($conflicting_leave_id) {
                $overlap_error_occurred = true; 
                $conflicting_post_title = get_the_title($conflicting_leave_id);
                $conflicting_start_date = get_post_meta($conflicting_leave_id, '_leave_start_date', true);
                $conflicting_end_date   = get_post_meta($conflicting_leave_id, '_leave_end_date', true);
                $conflicting_status_key = get_post_meta($conflicting_leave_id, '_leave_status', true);
                $statuses_map           = EMS_Leave_Options::get_leave_statuses();
                $conflicting_status_label = $statuses_map[$conflicting_status_key] ?? esc_html($conflicting_status_key);

                $overlap_message = sprintf(
                    __( 'Error: This leave request (Dates: %s to %s) overlaps with an existing active leave request: "%s" (ID: %s, Status: %s, Dates: %s - %s). Please cancel or modify the conflicting request, or change the dates/details of this request.', 'emmansys' ),
                    esc_html($start_date_val),
                    esc_html($end_date_val),
                    esc_html($conflicting_post_title),
                    $conflicting_leave_id,
                    esc_html($conflicting_status_label),
                    esc_html($conflicting_start_date),
                    esc_html($conflicting_end_date)
                );
                set_transient('ems_admin_leave_overlap_error', $overlap_message, 30); 
            }
        }
        
        if ( !empty( $errors ) || $overlap_error_occurred ) {
            if (!empty($errors)) { 
                foreach ( $errors as $error_msg ) {
                    add_settings_error( 'ems_add_leave_notice', 'validation_error', $error_msg, 'error' );
                }
                if (count(get_settings_errors('ems_add_leave_notice')) > 0) {
                    set_transient('settings_errors', get_settings_errors('ems_add_leave_notice'), 30);
                }
            }
            
            set_transient('ems_add_leave_form_data', $form_data, 30); 
            wp_safe_redirect( admin_url( 'edit.php?post_type=leave_request&page=ems-add-new-leave-request' ) );
            exit;
        }

        $new_leave_title = $this->_generate_next_leave_request_id();
        $employee_post = get_post($employee_cpt_id);
        $employee_name = $employee_post ? $employee_post->post_title : __('Unknown Employee', 'emmansys');
        $linked_wp_user_id = $employee_post ? get_post_meta($employee_cpt_id, '_employee_user_id', true) : null;
        // The author of the leave request post should be the WP user linked to the employee CPT,
        // or if not available (e.g. employee record not linked), then the current admin creating it.
        $post_author_id = $linked_wp_user_id ? $linked_wp_user_id : get_current_user_id();


        $leave_request_args = array(
            'post_title'  => $new_leave_title,
            'post_status' => 'publish', 
            'post_type'   => 'leave_request',
            'post_author' => $post_author_id, 
        );

        $new_leave_request_id = wp_insert_post( $leave_request_args, true );

        if ( is_wp_error( $new_leave_request_id ) ) {
            add_settings_error( 'ems_add_leave_notice', 'insert_error', __( 'Failed to create leave request: ', 'emmansys' ) . $new_leave_request_id->get_error_message(), 'error' );
            set_transient('settings_errors', get_settings_errors('ems_add_leave_notice'), 30);
            set_transient('ems_add_leave_form_data', $form_data, 30);
            wp_safe_redirect( admin_url( 'edit.php?post_type=leave_request&page=ems-add-new-leave-request' ) );
            exit;
        ...
        update_post_meta( $new_leave_request_id, '_leave_employee_cpt_id', $employee_cpt_id );
        update_post_meta( $new_leave_request_id, '_leave_employee_name', $employee_name );
        if ($linked_wp_user_id) { // Store the linked WP user ID if available from the employee CPT
            update_post_meta( $new_leave_request_id, '_leave_user_id', $linked_wp_user_id );
        } else { // If employee CPT not linked, store the creator's ID as the _leave_user_id (admin scenario)
             update_post_meta( $new_leave_request_id, '_leave_user_id', get_current_user_id() );
        }
        update_post_meta( $new_leave_request_id, '_leave_type', $leave_type_key );
        update_post_meta( $new_leave_request_id, '_leave_start_date', $start_date_val );
        update_post_meta( $new_leave_request_id, '_leave_end_date', $end_date_val );
        update_post_meta( $new_leave_request_id, '_leave_duration', $duration_key );
        update_post_meta( $new_leave_request_id, '_leave_reason', $reason );
        update_post_meta( $new_leave_request_id, '_leave_status', $status ); 
        update_post_meta( $new_leave_request_id, '_leave_admin_notes', $admin_notes );

        if ($status === 'approved') {
            $this->_handle_leave_balance_update_on_status_change($new_leave_request_id, 'newly_created', 'approved');
        }
        
        add_settings_error('ems_leave_notice', 'success_create', sprintf(__( 'Leave request %s created successfully.', 'emmansys' ), $new_leave_title ), 'success');
        set_transient('settings_errors', get_settings_errors('ems_leave_notice'), 30); 
        wp_safe_redirect( admin_url( 'edit.php?post_type=leave_request' ) );
        exit;
    }

  
    public function handle_profile_leave_request_submission() { /* ... same as v1.1.19 ... */
        ...
        if (!$post_title) {
            $post_title = __('Leave Request', 'emmansys') . ' ' . time(); // Fallback
            error_log("EmManSys: Failed to generate auto-increment ID for leave request during non-AJAX profile submission.");
        }
 ...
 * UPDATE HISTORY:
 * =====================================================================================
 * Version 1.1.20 (Current - Restrict Employee Field on Admin Add New Leave for Non-Admins)
 * - Modified `render_add_new_leave_request_page`:
 * - Checks if current user can `edit_others_leave_requests`.
 * - If not, the "Employee" field becomes a read-only display of their linked employee CPT record.
 * - A hidden input `ems_leave_employee_cpt_id` is used to pass the ID.
 * - If the user cannot edit others' leave and is not linked to an employee record, a message is shown, and form submission is disabled.
 * - Modified `handle_admin_add_new_leave_request`:
 * - Retrieves `ems_leave_employee_cpt_id` based on user capability (dropdown for admin, hidden/derived for others).
 * - Added specific error message if a non-admin user is not linked to an employee record.
 * - Ensures `post_author` for the leave request is the WP user linked to the Employee CPT, or the admin creating it if no link.
 * - Incremented plugin version to 1.1.20.
 *
 * Version 1.1.19
 * - Modified `handle_admin_add_new_leave_request`:
 * - If an overlap is detected, a detailed error message is stored in a specific transient `ems_admin_leave_overlap_error`.
 * - Other validation errors (e.g., empty fields) continue to use `add_settings_error` with the group `ems_add_leave_notice` and are stored in the generic `settings_errors` transient.
 * - Form data is repopulated via `ems_add_leave_form_data` transient on any error.
 * - Modified `render_add_new_leave_request_page`:
 * - Now first checks for and displays the `ems_admin_leave_overlap_error` transient.
 * - Then calls `settings_errors('ems_add_leave_notice')` to display any other validation errors.
 * - This provides a clearer, dedicated notification for overlap errors on the custom admin page.
 * - Incremented plugin version to 1.1.19.
 *
