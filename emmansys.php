...
final class Employee_Management_System {
    ...
    public function render_leave_request_details_meta_box( $post ) { 
        ...
        // Use EMS_Leave_Options class for these
    ...
    public function render_leave_request_columns( $column, $post_id ) { 
        ...
        switch ( $column ) { 
            ...
            case 'ems_leave_type': 
                ...
                echo esc_html( $leave_types_map[$type_key] ?? $type_key ); 
                break; 
    ...
    // REMOVED get_leave_types() and get_leave_statuses() from here as they are now in EMS_Leave_Options class
    ...
    public function show_leave_management_on_profile( $user_object_from_hook, $is_dashboard_context = false ) { 
        ...
        if ( $is_dashboard_context ) {
            ...
            $user_to_display = $user_object_from_hook;
             if ( ! ($user_to_display instanceof WP_User) || ! $user_to_display->ID ) { // Validate passed user object for profile page
                // Don't output error on other users' profiles if current user can't see their history anyway
                return; 
            }
        }
        ...
        <form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>" id="<?php echo esc_attr($form_id); ?>">
            ...
            <table class="form-table"><tbody>
                <tr><th><label for="<?php echo esc_attr($form_id); ?>-leave-type"><?php _e( 'Leave Type', 'emmansys' ); ?></label></th><td><select id="<?php echo esc_attr($form_id); ?>-leave-type" name="ems_profile_leave_type" required><option value=""><?php _e( '-- Select Type --', 'emmansys' ); ?></option><?php foreach ( $leave_types as $key => $label ) : ?><option value="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></td></tr>
        ...
        // Always include user ID based query if we are showing history for a specific user
        $history_args['meta_query'][] = array( 'key' => '_leave_user_id', 'value' => $target_profile_user_id, 'compare' => '=', 'type' => 'NUMERIC');
        
        // If only one condition, remove relation 'OR'
        if (count($history_args['meta_query']) === 1 && isset($history_args['meta_query']['relation'])) {
            unset($history_args['meta_query']['relation']);
        } elseif (empty($history_args['meta_query'])) { // If no conditions (e.g., no linked CPT and not targeting specific user)
             $history_args['author'] = $target_profile_user_id; // Fallback to post_author for very old requests
        }
        ...
        if ( $user_leave_requests->have_posts() ) : ?>
            ...
            <?php 
            // Ensure maps are available for the loop
            ...
            while ( $user_leave_requests->have_posts() ) : $user_leave_requests->the_post(); $request_id = get_the_ID(); ?>
                <tr>
                    ...
                    <td><?php echo esc_html( $leave_types_map_history[get_post_meta( $request_id, '_leave_type', true )] ?? get_post_meta( $request_id, '_leave_type', true ) ?: __('N/A', 'emmansys') ); ?></td>
 ...
 * UPDATE HISTORY:
 * =====================================================================================
 * Version 1.1.2 (Current - With Fixes for EMS_Leave_Options integration)
