...
final class Employee_Management_System {
        ...
        if ( 'post.php' === $hook_suffix || 'post-new.php' === $hook_suffix ) {
            ...
            if ( $screen && 'leave_request' === $screen->post_type ) {
                ...
                wp_localize_script('ems-admin-leave-validation', 'ems_leave_admin_data', array( 
                    ...
                    'can_select_past_dates' => current_user_can('approve_leave_requests') // New flag
                ));
            }
        } 
    ...
    public function save_leave_request_meta_data( $post_id, $post, $update ) { 
        ...
        // Allow past dates for admins/managers, otherwise restrict for new requests
 ...
 * UPDATE HISTORY:
 * =====================================================================================
 * Version 1.1.9 (Current - Admin Past Date Selection)
 * - Modified `save_leave_request_meta_data()` to allow users with 'approve_leave_requests' capability to select past start dates for new leave requests.
 * - Updated `enqueue_admin_scripts()` to localize a new `can_select_past_dates` flag for `admin-leave-validation.js`.
 * - (Assumes `admin-leave-validation.js` will be updated to use this flag to adjust datepicker `min` attribute).
 * - Updated `render_leave_request_details_meta_box` to conditionally set `min` attribute on date inputs based on user capability.
 * - Incremented plugin version to 1.1.9.
 * * Version 1.1.8
 ...
 * - Integrated overlap check into leave submission handlers.
 *
 * Version 1.1.6
