...
final class Employee_Management_System {
    ...
    public function render_leave_request_details_meta_box( $post ) { 
        ...
        if (strpos(strtolower($display_title), 'auto draft') !== false || empty($display_title) || strpos($display_title, 'LR') !== 0) {
            // If it's an auto-draft, empty, or not yet our LR format, show placeholder
            $display_title = __('(Will be auto-generated on save)', 'emmansys');
        }
    ...
    public function save_leave_request_meta_data( $post_id, $post, $update ) { 
        ...
        $submitted_status = isset($_POST['ems_leave_status']) ? sanitize_key($_POST['ems_leave_status']) : $old_status; 
        ...
        if ($employee_cpt_id_from_form && $start_date_val && $end_date_val && !$is_autosave) {
            ...
            if ($conflicting_leave_id) {
                ...
                $conflicting_status_label = $statuses_map[$conflicting_status_key] ?? $conflicting_status_key;
                ...
                add_settings_error('ems_leave_notice', 'overlap_error_admin_forced_draft', $error_message, 'error');
        ...
        // Title Generation/Update for Leave Requests
        // This ensures new admin-created requests get an LR ID, and existing ones are not re-titled if they already have an LR ID.
        $current_post_for_title = get_post($post_id); // Get the most current post object
        if ($current_post_for_title && strpos($current_post_for_title->post_title, 'LR') !== 0) {
            // If the title doesn't start with 'LR', it means it's either 'Auto Draft', empty, or something else.
            // This is most likely for a new post created via admin "Add New" that hasn't had its title set yet.
            // Or an old post that somehow missed getting an LR title.
            if (!$is_autosave) { // Only generate new title on manual save
 ...
 * UPDATE HISTORY:
 * =====================================================================================
 * Version 1.1.14 (Current - Refined Overlap & Auto Draft Handling for Admin)
 * - In `save_leave_request_meta_data()`:
 * - If an overlap is detected during manual save by an admin:
 * - An error notice is set using `add_settings_error` and stored in a transient.
 * - If the admin was trying to save the leave as 'pending' or 'approved', the status is now forced to 'draft'.
 * - The save process continues (allowing title update from "Auto Draft").
 * - The admin is redirected back to the edit screen with the notice and the leave set to 'draft'.
 * - Refined title generation for new admin-created leave requests to ensure 'Auto Draft' is replaced by 'LRXXXX' ID.
 * - Incremented plugin version to 1.1.14.
 * * Version 1.1.13
 * - Added `_generate_next_leave_request_id()` method to create titles like 'LR00000001'.
 * - Added WordPress option `ems_last_leave_request_id_counter` to track the last ID.
 * - Updated `activate()` to initialize the counter option.
 * - Modified `ajax_handle_profile_leave_submission()` and `handle_profile_leave_request_submission()` to use the new ID for `post_title` when inserting new leave requests.
 * - Modified `save_leave_request_meta_data()` to generate and set the auto-incrementing title for new leave requests created directly in the admin (if title is 'auto draft').
 *
 * Version 1.1.12
 * - Refined overlap handling in `save_leave_request_meta_data` to set conflicting leaves to 'draft' and show an admin notice instead of `wp_die()`.
 * - Improved "Auto Draft" title replacement logic.
 *
