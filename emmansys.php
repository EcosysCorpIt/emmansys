...
final class Employee_Management_System {
    ...
    public function enqueue_admin_scripts( $hook_suffix ) {
        ...
        if ( 'profile.php' === $hook_suffix || 'user-edit.php' === $hook_suffix ) {
            wp_enqueue_script( 'ems-profile-leave-validation', EMS_PLUGIN_URL . 'js/profile-leave-validation.js', array( 'jquery' ), self::VERSION, true );
             wp_localize_script('ems-profile-leave-validation', 'ems_leave_data', array(
                'today' => current_time('Y-m-d'),
            ));
        }
    }
    ...
    public function enqueue_frontend_scripts() {
        // No frontend scripts needed for this specific request yet.
    }
    ...
    public function activate() {
        ...
        if (!file_exists($js_admin_dir . 'admin-leave-validation.js')) {
            file_put_contents($js_admin_dir . 'admin-leave-validation.js', '// Admin Leave Validation Script');
        }
        if (!file_exists($js_admin_dir . 'profile-leave-validation.js')) {
            file_put_contents($js_admin_dir . 'profile-leave-validation.js', '// Profile Leave Validation Script');
        }
    }
    ...
    public function register_leave_request_cpt() {
        // ... (capability_type and capabilities same as v1.0.9) ...
    ...
    public function add_leave_request_meta_boxes() {
        // ... (same as v1.0.9 with capability check) ...
    ...
    public function get_leave_durations() {
        return array(
    ...
    public function render_leave_request_details_meta_box( $post ) {
        ...
        $statuses = $this->get_leave_statuses();
        $durations = $this->get_leave_durations();
    ...
    public function save_leave_request_meta_data( $post_id, $post ) { /* ... same as v1.0.9 with date validation ... */ if ( ! isset( $_POST['ems_leave_request_details_nonce'] ) || ! wp_verify_nonce( $_POST['ems_leave_request_details_nonce'], 'ems_save_leave_request_details' ) ) return; if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return; if ( $post->post_type !== 'leave_request' ) return; if ( !current_user_can( 'edit_leave_request', $post_id ) ) { return; }
    ...
    public function set_leave_request_columns( $columns ) { 
        ...
        $new_columns['ems_leave_duration_col'] = __( 'Duration', 'emmansys' ); // New column
    ...
    public function show_leave_management_on_profile( $user_profile ) { 
        if ( !current_user_can('view_own_profile_leave_history') && get_current_user_id() != $user_profile->ID) { return; } 
        ...
        <form method="post" action="<?php echo esc_url( admin_url('admin-post.php') ); ?>" id="ems-profile-leave-form">
            ...
            <table class="form-table"><tbody>
                <tr><th><label for="ems_profile_leave_type"><?php _e( 'Leave Type', 'emmansys' ); ?></label></th><td><select id="ems_profile_leave_type" name="ems_profile_leave_type" required><option value=""><?php _e( '-- Select Type --', 'emmansys' ); ?></option><?php foreach ( $this->get_leave_types() as $key => $label ) : ?><option value="<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $label ); ?></option><?php endforeach; ?></select></td></tr>
        ...
        <?php endif; ?>
        <?php if (current_user_can('view_own_profile_leave_history') && get_current_user_id() == $user_profile->ID ) : ?>
        <h3><?php _e( 'Your Leave Request History', 'emmansys' ); ?></h3>
        <?php
        $args = array('post_type' => 'leave_request', 'posts_per_page' => -1, 'meta_query' => array('relation' => 'OR', array( 'key' => '_leave_employee_cpt_id', 'value' => $linked_employee_cpt_id, 'compare' => '=', 'type' => 'NUMERIC'), array( 'key' => '_leave_user_id', 'value' => $user_profile->ID, 'compare' => '=', 'type' => 'NUMERIC')), 'orderby' => 'date', 'order' => 'DESC',);
        if (!$linked_employee_cpt_id) { $args['meta_query'] = array(array( 'key' => '_leave_user_id', 'value' => $user_profile->ID, 'compare' => '=', 'type' => 'NUMERIC'));}
        ...
        if ( $user_leave_requests->have_posts() ) : ?>
            ...
            <?php $leave_types_map = $this->get_leave_types(); $statuses_map = $this->get_leave_statuses(); $durations_map = $this->get_leave_durations();
            while ( $user_leave_requests->have_posts() ) : $user_leave_requests->the_post(); $request_id = get_the_ID(); ?>
                <tr>
                    ...
                    <td><?php echo esc_html( $leave_types_map[get_post_meta( $request_id, '_leave_type', true )] ?? get_post_meta( $request_id, '_leave_type', true ) ); ?></td>
                    ...
                    <td><?php echo esc_html( $durations_map[get_post_meta( $request_id, '_leave_duration', true )] ?? get_post_meta( $request_id, '_leave_duration', true ) ); ?></td>
                    <td><strong><?php echo esc_html( $statuses_map[get_post_meta( $request_id, '_leave_status', true )] ?? get_post_meta( $request_id, '_leave_status', true ) ); ?></strong></td>
                    <td><?php echo wp_kses_post( get_post_meta( $request_id, '_leave_reason', true ) ); ?></td>
                </tr>
            <?php endwhile; ?>
            </tbody></table>
        <?php else : ?> <p><?php _e( 'You have not submitted any leave requests yet.', 'emmansys' ); ?></p>
