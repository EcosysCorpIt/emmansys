...
final class Employee_Management_System {
    ...
    private function init_hooks() {
        ...
        add_action( 'wp_ajax_ems_change_leave_status', array( $this, 'ajax_ems_change_leave_status' ) ); // New AJAX handler for status change
    ...
    public function enqueue_admin_scripts( $hook_suffix ) { 
        ...
        // Enqueue script for leave request list table actions
    ...
    public function activate() { 
        ...
        // Create dummy JS files if they don't exist to prevent enqueue errors on fresh install
        if (!file_exists($js_dir . 'admin-leave-validation.js')) {
            file_put_contents($js_dir . 'admin-leave-validation.js', '// Admin Leave Validation Script');
        }
        if (!file_exists($js_dir . 'profile-leave-validation.js')) {
            file_put_contents($js_dir . 'profile-leave-validation.js', '// Profile/Dashboard Leave Validation Script');
        }
        if (!file_exists($js_dir . 'admin-leave-list-actions.js')) {
            file_put_contents($js_dir . 'admin-leave-list-actions.js', '// Admin Leave List Actions Script');
        }
    }
     ...
     * Plugin deactivation.
     */
    public function deactivate() { /* ... same as 1.1.7 ... */ flush_rewrite_rules(); }
     ...
     *
     * @since 1.1.8
     * @param int    $leave_request_id The ID of the leave request.
     * @param string $old_status The status of the leave request before the change.
     * @param string $new_status The new status of the leave request.
     * @return bool True if balance was potentially updated, false otherwise.
     */
    private function _handle_leave_balance_update_on_status_change($leave_request_id, $old_status, $new_status) {
        $leave_type_key   = get_post_meta($leave_request_id, '_leave_type', true);
        $leave_start_date = get_post_meta($leave_request_id, '_leave_start_date', true);
        $leave_end_date   = get_post_meta($leave_request_id, '_leave_end_date', true);
        $leave_duration_key = get_post_meta($leave_request_id, '_leave_duration', true);
        $employee_id      = get_post_meta($leave_request_id, '_leave_employee_cpt_id', true);
        ...
        $tracks_balance = $current_leave_type_definition && 
                          ( (isset($current_leave_type_definition['initial_balance']) && $current_leave_type_definition['initial_balance'] > 0) || $leave_type_key !== 'unpaid');

        if (!$tracks_balance) {
            return false; // This leave type does not track balances (e.g., unpaid or 0 initial and not configured otherwise)
        }
        ...
        $previously_deducted_days = (float) get_post_meta($leave_request_id, '_ems_deducted_leave_days', true);

        if ($new_status === 'approved' && $old_status !== 'approved') {
            // Newly Approved: Deduct days
            if ($days_for_this_request > 0) {
                ...
                update_post_meta($leave_request_id, '_ems_deducted_leave_days', $days_for_this_request);
                return true;
            }
        } elseif ($old_status === 'approved' && $new_status !== 'approved') {
            // No longer Approved (was approved): Credit back days
            if ($previously_deducted_days > 0) {
                $new_balance = $current_employee_balance + $previously_deducted_days;
                update_post_meta($employee_id, $balance_meta_key, $new_balance);
                delete_post_meta($leave_request_id, '_ems_deducted_leave_days');
                return true;
            }
        } elseif ($new_status === 'approved' && $old_status === 'approved') {
            // Remained Approved, but details might have changed (e.g. dates, duration)
            $new_deduction_amount = $days_for_this_request;
            if ($new_deduction_amount != $previously_deducted_days) {
                $adjustment = $previously_deducted_days - $new_deduction_amount; // If new is less, adjustment is positive (credit back). If new is more, adjustment is negative (deduct more).
                $new_balance = $current_employee_balance + $adjustment;
                update_post_meta($employee_id, $balance_meta_key, $new_balance);
                update_post_meta($leave_request_id, '_ems_deducted_leave_days', $new_deduction_amount);
                return true;
            }
        }
        return false; // No balance update occurred
    }
    ...
    public function save_leave_request_meta_data( $post_id, $post, $update ) { 
        ...
        if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE && !$update ) return;  
    ...
    public function set_leave_request_columns( $columns ) { 
        ...
        $new_columns['ems_leave_actions'] = __( 'Actions', 'emmansys' ); // New column
    ...
    public function render_leave_request_columns( $column, $post_id ) { 
        ...
        switch ( $column ) { 
            ...
            case 'ems_leave_status': 
                ...
                echo '<strong id="ems-status-text-' . esc_attr($post_id) . '">' . esc_html( $statuses_map[$status_key] ?? $status_key ) . '</strong>'; 
                break;
            case 'ems_leave_actions':
                if ( current_user_can( 'approve_leave_requests' ) ) {
                    $current_status = get_post_meta( $post_id, '_leave_status', true );
                    echo '<div class="ems-leave-status-changer" data-leave-id="' . esc_attr($post_id) . '">';
                    echo '<select name="ems_new_status" class="ems-new-status-select">';
                    foreach ( $statuses_map as $key => $label ) {
                        // Employee cannot set to pending or approved from here, only admin can.
                        // Employee can only cancel their own 'pending' requests (handled on profile/dashboard forms)
                        // Admin can set to any status.
                        if ($key === 'cancelled' && $current_status !== 'pending') {
                           // Don't show 'cancelled' if not pending, unless it's already cancelled
                           if ($current_status !== 'cancelled') continue;
                        }
                        printf(
                    ...
                    echo '</select>';
                    echo '<button type="button" class="button button-secondary ems-update-status-button" style="margin-left:5px;">' . esc_html__( 'Update', 'emmansys' ) . '</button>';
    ...
    public function make_leave_request_columns_sortable( $columns ) { /* ... same as v1.1.7 ... */ $columns['ems_leave_title_col'] = 'title'; $columns['ems_leave_employee'] = 'ems_leave_employee_sort'; $columns['ems_leave_user'] = 'ems_leave_user_sort'; $columns['ems_leave_type'] = 'ems_leave_type_sort'; $columns['ems_leave_status'] = 'ems_leave_status_sort'; return $columns; }
    public function sort_leave_request_columns_query( $query ) { /* ... same as v1.1.7 ... */ if ( ! is_admin() || ! $query->is_main_query() ) return; $orderby = $query->get( 'orderby' ); $post_type = $query->get('post_type'); if ($post_type === 'leave_request') { if ( 'ems_leave_employee_sort' === $orderby ) { $query->set( 'meta_key', '_leave_employee_name' ); $query->set( 'orderby', 'meta_value' ); } elseif ( 'ems_leave_user_sort' === $orderby ) { $query->set( 'meta_key', '_leave_user_id' ); $query->set( 'orderby', 'meta_value_num' ); } elseif ( 'ems_leave_type_sort' === $orderby ) { $query->set( 'meta_key', '_leave_type' ); $query->set( 'orderby', 'meta_value' ); } elseif ( 'ems_leave_status_sort' === $orderby ) { $query->set( 'meta_key', '_leave_status' ); $query->set( 'orderby', 'meta_value' ); } } }
    ...
    public function ajax_ems_change_leave_status() {
        check_ajax_referer( 'ems_change_leave_status_nonce', 'nonce' );

        if ( ! current_user_can( 'approve_leave_requests' ) ) {
            wp_send_json_error( array( 'message' => __( 'You do not have permission to change leave statuses.', 'emmansys' ) ) );
        }

        $leave_request_id = isset( $_POST['leave_id'] ) ? intval( $_POST['leave_id'] ) : 0;
        $new_status       = isset( $_POST['new_status'] ) ? sanitize_key( $_POST['new_status'] ) : '';

        if ( ! $leave_request_id || empty( $new_status ) ) {
            wp_send_json_error( array( 'message' => __( 'Invalid data provided.', 'emmansys' ) ) );
        }

        $valid_statuses = array_keys( EMS_Leave_Options::get_leave_statuses() );
        if ( ! in_array( $new_status, $valid_statuses ) ) {
            wp_send_json_error( array( 'message' => __( 'Invalid status selected.', 'emmansys' ) ) );
        }
        
        $old_status = get_post_meta( $leave_request_id, '_leave_status', true );

        // Prevent employee from changing status to anything other than 'cancelled' if it's 'pending'
        // This check is more for direct AJAX calls, UI should restrict options.
        if ( !current_user_can('approve_leave_requests') ) {
            $current_employee_id = get_post_meta($leave_request_id, '_leave_employee_cpt_id', true);
            ...
            if (!empty($linked_employee_posts)) {
                $user_employee_id = $linked_employee_posts[0];
            }

            if ($current_employee_id != $user_employee_id || ($new_status !== 'cancelled' || $old_status !== 'pending')) {
                 wp_send_json_error( array( 'message' => __( 'You are not authorized to make this status change.', 'emmansys' ) ) );
            }
        }


        $updated = update_post_meta( $leave_request_id, '_leave_status', $new_status );

        if ( $updated ) {
            // Handle balance update
            $this->_handle_leave_balance_update_on_status_change( $leave_request_id, $old_status, $new_status );
            
            $statuses_map = EMS_Leave_Options::get_leave_statuses();
            wp_send_json_success( array( 
                'message' => __( 'Leave status updated successfully.', 'emmansys' ),
                'new_status_label' => $statuses_map[$new_status] ?? $new_status
            ) );
        } else {
            // This can happen if the new status is the same as the old one.
            // Or if update_post_meta truly failed.
             $meta_check = get_post_meta( $leave_request_id, '_leave_status', true );
             if ($meta_check === $new_status && $old_status === $new_status) {
                 wp_send_json_success( array( 
                    'message' => __( 'Leave status is already set to the selected value.', 'emmansys' ),
                    'new_status_label' => EMS_Leave_Options::get_leave_statuses()[$new_status] ?? $new_status
                ) );
             } else {
                wp_send_json_error( array( 'message' => __( 'Failed to update leave status.', 'emmansys' ) ) );
             }
        }
    }
 ...
 * UPDATE HISTORY:
 * =====================================================================================
 * Version 1.1.8 (Current - Inline Status Change in Admin List)
 * - Added "Actions" column to Leave Requests list table for quick status changes.
 * - Implemented AJAX functionality for updating leave status directly from the list table.
 * - Added `js/admin-leave-list-actions.js` for handling AJAX and UI updates.
 * - Added `ajax_ems_change_leave_status` AJAX handler in PHP.
 * - Refactored leave balance update logic into `_handle_leave_balance_update_on_status_change()` method,
 * called by both `save_leave_request_meta_data` and the new AJAX handler.
 * - Enqueued the new JS file and localized necessary data on the leave request list screen.
 * - Incremented plugin version to 1.1.8.
 *
 * Version 1.1.7
 * - Added `has_overlapping_active_leave()` helper method.
 * - Integrated overlap check into leave submission handlers (`ajax_handle_profile_leave_submission`, 
 * `handle_profile_leave_request_submission`, and `save_leave_request_meta_data`).
 *
 * Version 1.1.6
 * - Added `calculate_leave_request_days()` helper method.
 * - Modified `save_leave_request_meta_data()` to deduct/credit leave balances upon approval/status change.
 *
 * Version 1.1.5
 * - Updated `show_leave_management_on_profile` to improve display of leave balances.
 *
 * Version 1.1.4
